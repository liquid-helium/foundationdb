...
struct AuditStorageState {
	...
	AuditStorageState() : auditServerId(UID()), type(0), phase(0), ddId(UID()) {}
...
struct TriggerAuditRequest {
	...
	TriggerAuditRequest(AuditType type, KeyRange range, KeyValueStoreType engineType)
	  : type(static_cast<uint8_t>(type)), range(range), engineType(engineType), cancel(false) {}

	TriggerAuditRequest(AuditType type, UID id) : id(id), type(static_cast<uint8_t>(type)), cancel(true) {}
